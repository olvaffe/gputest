...
tex_test_init_pipeline(struct tex_test *test)
{
    ...
    d3d11->result = ID3D11Device5_CreateVertexShader(d3d11->dev, tex_test_vs, sizeof(tex_test_vs),
                                                     NULL, &test->vs);
    d3d11_check(d3d11, "CreateVertexShader");

    d3d11->result = ID3D11Device5_CreatePixelShader(d3d11->dev, tex_test_ps, sizeof(tex_test_ps),
                                                    NULL, &test->ps);
...
tex_test_init_tex(struct tex_test *test)
{
    ...
    d3d11->result = ID3D11Device5_CreateShaderResourceView(
        d3d11->dev, (ID3D11Resource *)test->tex, NULL, &test->srv);
    ...
    d3d11->result = ID3D11Device5_CreateSamplerState(d3d11->dev, &sampler_desc, &test->sampler);
...
tex_test_draw(struct tex_test *test)
{
    ...
    ID3D11DeviceContext4_PSSetShaderResources(d3d11->ctx, 0, 1, &test->srv);
    ID3D11DeviceContext4_PSSetSamplers(d3d11->ctx, 0, 1, &test->sampler);

    ID3D11DeviceContext4_OMSetRenderTargets(d3d11->ctx, 1, &test->rtv, NULL);

    ID3D11DeviceContext4_Draw(d3d11->ctx, ARRAY_SIZE(tex_test_vertices), 0);
