...
ubo_test_init_pipeline(struct ubo_test *test)
{
    ...
    vk_set_pipeline_shaders(vk, test->pipeline, ubo_test_vs, sizeof(ubo_test_vs), ubo_test_fs,
                            sizeof(ubo_test_fs));
...
ubo_test_init_ubo(struct ubo_test *test)
{
    ...
    test->ubo = vk_create_buffer(vk, sizeof(transform), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
